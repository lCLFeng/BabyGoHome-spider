#!/usr/bin/python3
# -*-coding:utf-8-*-
# project :get_latitude_ longitude
# file    :exchange.py
# author  :lCLFeng
# time    :2018/6/8 10:28
# editor  :PyCharm 
...
def read_excel():
    ...
    for row_number in range(1, sheet.nrows):
        ...
        # 失踪人现在所在地
        now_location = row_values[0] + row_values[1]  # 将省和市拼接在一起
        ...
        # 失踪人原来所在地
        origin_location = row_values[4] + row_values[5]
        geocode_result = geocode(origin_location)
        if geocode_result is not None:
            origin_location_latlng = geocode_result.split(',')
        else:
            print('\033[1;31;m' + 'None' + '\033[0m')
            continue
        # 将数据添加到row_values_new中
        row_values_new.append(origin_location)
        row_values_new.append(origin_location_latlng[0])
        row_values_new.append(origin_location_latlng[1])
        # 控制台输出row_values_new
        ...
        worksheet.write(row_number, 3, row_values_new[3])
        worksheet.write(row_number, 4, row_values_new[4])
        worksheet.write(row_number, 5, row_values_new[5])
        # 保存excel
